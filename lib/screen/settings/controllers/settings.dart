...
class Settings {
  ...
  static const bool _isSetMemberClose = false;

  static const bool _isProfileAdd = false;

  static const bool _isOnBoardShow = false;

  static const bool _isFireseLogin = false;
  ...
  static void reset(WidgetRef ref) {
    ...
    ref.read(isOnboardShowProvider.notifier).reset();
    ref.read(isFireBaseLoginProvider.notifier).reset();
    ref.read(setAddProfileProvider.notifier).reset();
    ref.read(setMemberScreenProvider.notifier).reset();
  ...
  static void init(Ref ref) {
    ...
    // Use material 3, theme mode and active color scheme.
    ref.read(isOnboardShowProvider.notifier).init();
    ref.read(isFireBaseLoginProvider.notifier).init();
    ref.read(setAddProfileProvider.notifier).init();
    ref.read(setMemberScreenProvider.notifier).init();
  ...
  static const String _keyIsFireBaseLogin = 'keyIsFireBaseLogin ';
  ...
  static final NotifierProvider<SettingsEntry<bool>, bool>
      isFireBaseLoginProvider = NotifierProvider<SettingsEntry<bool>, bool>(
    () {
      return SettingsEntry<bool>(
        defaultValue: _isFireseLogin,
        key: _keyIsFireBaseLogin,
      );
    },
    // Use the unique key-value DB key as provider name, useful for debugging.
    name: '${_keyIsFireBaseLogin}Provider',
  );

  /// String key used for defining if we use Material 3 or Material 2.
  ///
  /// The associated provider uses same name with "Provider" added to it.
  static const String _keyIsOneBoardShow = 'keyIsOneBoardShow';

  /// Provider for swapping primary and secondary colors in light theme mode.
  ///

  static final NotifierProvider<SettingsEntry<bool>, bool>
      isOnboardShowProvider = NotifierProvider<SettingsEntry<bool>, bool>(
    () {
      return SettingsEntry<bool>(
        defaultValue: _isOnBoardShow,
        key: _keyIsOneBoardShow,
      );
    },
    // Use the unique key-value DB key as provider name, useful for debugging.
    name: '${_keyIsOneBoardShow}Provider',
  );

  /// String key used for defining if we use Material 3 or Material 2.
  ///
  /// The associated provider uses same name with "Provider" added to it.
  static const String _keyAddProfileData = 'keyAddProfileData';

  /// Provider for swapping primary and secondary colors in light theme mode.
  ///

  static final NotifierProvider<SettingsEntry<bool>, bool>
      setAddProfileProvider = NotifierProvider<SettingsEntry<bool>, bool>(
    () {
      return SettingsEntry<bool>(
        defaultValue: _isProfileAdd,
        key: _keyAddProfileData,
      );
    },
    // Use the unique key-value DB key as provider name, useful for debugging.
    name: '${_keyAddProfileData}Provider',
  );

  /// String key used for defining if we use Material 3 or Material 2.
  ///
  /// The associated provider uses same name with "Provider" added to it.
  static const String _keySetMemberScreen = 'setMemberScreen';

  /// Provider for swapping primary and secondary colors in light theme mode.
  ///

  static final NotifierProvider<SettingsEntry<bool>, bool>
      setMemberScreenProvider = NotifierProvider<SettingsEntry<bool>, bool>(
    () {
      return SettingsEntry<bool>(
        defaultValue: _isSetMemberClose,
        key: _keySetMemberScreen,
      );
    },
    // Use the unique key-value DB key as provider name, useful for debugging.
    name: '${_keySetMemberScreen}Provider',
  );
